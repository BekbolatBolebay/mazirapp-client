...
- Жүйе **PocketBase Auth** қолданады.
- Пайдаланушының **role** бағаны `admin` немесе `super_admin` болуы тиіс.
- API-ге сұраныс жібергенде `pb.authStore` арқылы автоматты түрде тексереді.
...
2. **Суреттерді жүктеу:** Суреттер үшін ішкі PocketBase Storage дайын тұр. API: `/api/upload`.
3. **Real-time:** Тапсырыстарды бақылау үшін PocketBase-тің real-time мүмкіндігін қолдану керек (`pb.collection('orders').subscribe('*', ...)`).
